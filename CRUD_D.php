...
$conn = mysqli_connect($servername,$username,$pwd,$db);
 ...
 if($_SERVER["REQUEST_METHOD"]=="GET"){
	 ...
	 if(isset($_GET["did"])){
		 $txtid=$_GET["did"];
		  mysqli_query($conn,"delete from tbl where id='". $txtid ."'");
		  $errsave="Record Deleted";
		  $txtid ="";
		  //Yun lang para sa CRUD - Delete... Tpos n ung CRUD.. Basic nga lang ung gnw ntin para mas mabilis n magwan ng sample. Upload ko ung file para matest at mas mapagaralan nyo. 4 n file para sa CRUD. Yun lng at salamat (^__^)
	 }
 }
...
<table border="0">
  ...
  <?php
  /*
    Next is yung pggmit ng limit at offset sa mysql o s sql statement.
	Sa table ntin meron tyong 5 n records
	Ididisplay ntin ung 3 record lang simula sa id n 1. 
	Lumbas n ung 3 record simula sa una. Laging tatandaan n ang record index ay lging ngsisimula sa zero
	ung 3 nmn ay ung bilang kung hanggng saan ang kukunin. May mgkasunod n pngalan Mharc at Mhark
	Mharc ang huli, try ntin plabasin ung Mhark. 4 nga lang ang lumbas. 
	Yun lang. Gnun lang ang pggmit ng limit at offset.. Next topic ay ung paganation... Mggmit at ggmitin ntin ang limit at offset sa susunod n topic. Upload ko nlng yung file para mas mpag-araln at matest nyo sa unit nyo. Salamat (^__^)
  */
     $q = mysqli_query($conn,"select * from tbl limit 1,2");
	 while($r = mysqli_fetch_array($q)){
		 echo "<tr><td>" . $r["id"] . "</td>".
				   ...
				   <?php
				   /*
				   Next ntin is yung delete. Sa delete ggyhin o ggmitin ntin ung gnw natin sa edit at lalagyn natin ng konting javascript.
				   */
				   ?>
				   | 
				   <!-- So, ok n ung link o href ntin sa delete-->
		<a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF']).'?did='.$r['id'];?>" onclick="return confirm('Are you sure? Delete na?')">Delete </a></td></tr>
		 <?php
	 }
